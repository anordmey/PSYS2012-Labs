...
```{webr}
#| edit: false
anxiety_data <- read.csv("data/math_anxiety.csv")
...
What is the mean age for participants in the `anxiety_data` dataframe? Report your answer using a *complete sentence* including units (e.g., "The mean age of participants is _____ years.")
...
If you are stuck, review the `student_data` example above, and try to do the same thing here with `anxiety_data`. If you are getting an error, review your work for typos! Remember that capitalization matters in R!
...
For Question 3c, use the code block below to create a histogram of the math anxiety scores. A histogram is a plot that shows all possible values of a variable on the x-axis, and the frequency of those variables on the y-axis. For this question you don't need to copy your R code, you just need to copy/paste your histogram into your answer key!
...
``` r
hist(anxiety_data$math_anxiety)
```
